...
const NavLinks: React.FC<{ className?: string, onLinkClick?: () => void }> = ({ className, onLinkClick }) => (
  <nav className={className}>
    ...
    <NavLink to="/blog" onClick={onLinkClick} className={({ isActive }) => `px-4 py-2 rounded-md transition-colors duration-300 ${isActive ? 'text-brand-purple font-bold' : 'hover:bg-purple-50'}`}>Blog</NavLink>
